        ...
        @if (Auth()->user()->carros()->find($carro->id))
          <form action="{{route('site.favoritos.delete', $carro)}}" method="post">
            ...
            @can ('favoritos-delete')
              {{ csrf_field() }}
              {{ method_field('DELETE') }}
              <button title="Remover Favoritos" class="btn yellow"><i class="material-icons">stars</i></button>
            @endcan
          </form>
        @else
          <form action="{{route('site.favoritos.create', $carro)}}" method="post">
            ...
            @can ('favoritos-create')
              {{ csrf_field() }}
              <button title="Adicionar aos Favoritos" class="btn green"><i class="material-icons">star</i></button>
            @endcan
          </form>
        @endif
